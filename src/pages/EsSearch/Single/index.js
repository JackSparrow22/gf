...
@Form.create()
export default class VSingle extends PureComponent {
  ...
  normalSearch = (value) => {
    if (!value) return;
    //router push
    router.push(`/essearch/single/search/normal/${value}`);
  };

  advanceSearch = (value) => {
    if (!value) return;
    //router push
    router.push(`/essearch/single/search/advance/${value}`);
  };
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          ...
          <a onClick={() => this.normalSearch(record.id)}>普通查询</a>
          <Divider type="vertical"/>
          <a onClick={() => this.advanceSearch(record.id)}>高级查询</a>
          <Divider type="vertical"/>
