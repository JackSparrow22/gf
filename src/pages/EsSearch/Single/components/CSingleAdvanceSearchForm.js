...
class CSingleAdvanceSearchForm extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'singleAdvance/fetchSchema',
      payload: { id },
    });
  ...
  SelectItem = (field, key, mode = null) => {
    ...
    const { singleAdvance } = this.props;
    const { selectOptions } = singleAdvance;
    const placeholder = mode ? '请选择，可多选' : '请选择';
  ...
  goNormal = () => {
    const { id } = this.props;
    router.push(`/essearch/single/search/normal/${id}`);
  };
      ...
      <Form onSubmit={this.handleSubmit} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            ...
            <a style={{ marginLeft: 18 }} onClick={this.goNormal}>
              普通查询
            </a>
